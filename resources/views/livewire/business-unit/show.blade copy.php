<div class="container mx-auto px-4 py-6">
    @php
        function renderHierarchy($units, $level = 0, $component = null) {
            $html = '';
            foreach ($units as $unit) {
                ...
                $html .= '<span class="text-gray-400 font-mono text-sm mr-2">' . $indent . $bullet . '</span>';
                $html .= '<span class="font-medium text-gray-900">' . $unit->name . '</span>';
                $html .= '<span class="ml-2 text-sm text-gray-500">(' . $unit->code . ')</span>';
                ...
                if ($unit->children && $unit->children->count() > 0) {
                    $html .= renderHierarchy($unit->children, $level + 1, $component);
                }
            }
            return $html;
        }
                ...
                <div class="p-6">
                    <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
                        ...
                        <div>
                            ...
                            <dl class="space-y-3">
                                <div>
                                    <dt class="text-sm font-medium text-gray-500">Total Children</dt>
                                    <dd class="text-sm text-gray-900">{{ $unit->allChildren->count() }}</dd>
                                </div>
                                <div>
                                    <dt class="text-sm font-medium text-gray-500">Direct Children</dt>
                                    <dd class="text-sm text-gray-900">{{ $unit->children->count() }}</dd>
                                </div>
                                <div>
                                    <dt class="text-sm font-medium text-gray-500">Created At</dt>
                                    <dd class="text-sm text-gray-900">{{ $unit->created_at->format('Y-m-d H:i') }}</dd>
                                </div>
                            </dl>
                        ...
                        <div>
                            <h3 class="text-lg font-medium text-gray-900 mb-4">Complete Business Units Hierarchy</h3>
                                ...
                                <div class="mt-6">
                                    ...
                                    <div class="bg-yellow-50 border border-yellow-200 rounded-lg p-4">
                                        <div class="font-mono text-sm space-y-1">
                                            ...
                                            @endphp
                                            {!! renderHierarchy($currentUnitCollection, 0, $this) !!}
                                        </div>
                                    </div>
                                </div>
                            @endif
                            ...
                            @if($unit->children->count() > 0)
                                <div class="mt-6">
                                    <h4 class="text-md font-medium text-gray-700 mb-3">Direct Children Actions</h4>
                                    <div class="space-y-2">
